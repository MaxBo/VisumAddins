...
def Run(param):
    ...
    # Set SKIMMatrix parameters
    """als variablen übergeben?"""
    code = v+' '+tablename
    a.SetAttValue('Name', code)
    a.SetAttValue('Code', code)
    a.SetAttValue('MatrixType', 'MATRIXTYPE_SKIM')
    ...
    with tables.openFile(filepath, 'a') as h:
        try:
            try:
                table_in_hdf5 = h.getNode(h.root.put, tablename)
            except:pass
            try:
                table_in_hdf5 = h.getNode(h.root.iv, tablename)
            except:pass
            try:
                table_in_hdf5 = h.getNode(h.root.nmt, tablename)
            except: pass                
            
            zones = Visum.Net.Zones.GetAll
            tt = table_in_hdf5[0]
            #win32api.MessageBox(0,str(zones))
            f = fromNumArray(tt, zones)
            win32api.MessageBox(0,str(f))
            values = f['datavalues']
            SetMatrix(Visum, code, values)
    ...
    #for z, code in enumerate(zeit):
        ...
        #ovcode = v+code.encode("iso-8859-15")
            ...
            #a.SetAttValue('Name', v+' '+table)
            #a.SetAttValue('Code', v+' '+table)
            #a.SetAttValue('MatrixType', 'MATRIXTYPE_SKIM')
