    ...
    with tables.openFile(filepath, 'a') as h:
        ...
        for m in AllMatrices:
            ...
            if m_type =='MATRIXTYPE_SKIM':
                ...
                no = m.AttValue('No')
##                addIn.ReportMessage(_("Name: %s, code: %s, No: %s").decode('iso-8859-15') % (name, code, no))
...
##                h.root._v_attrs['VISUM Version'] = inputdata
        ...
        # Tabelle Bezirke
        try: zonetable = h.getNode(h.root.Bezirke)
        except NoSuchNodeError:
            zones = Visum.Net.Zones
            nummer = numpy.array(VisumPy.helpers.GetMulti(zones, 'No'))
            name =numpy.array(VisumPy.helpers.GetMulti(zones, 'Name'))
            rec = numpy.rec.fromarrays([nummer, name], names=['zone_no', 'zone_name'], titles=['Bezirke', 'BezNamen'], formats=['<i4','a36'])
